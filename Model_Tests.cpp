...
flmm.setY(y);
flmm.init();
double value = flmm.f(ldelta);
EXPECT_NEAR(value, 5.554877, 1e-3);
}

TEST(LMM, Train_null_model){
FaSTLMM flmm = FaSTLMM();
// MatrixXd X(10, 5);
// X << 0.8147,    0.1576,    0.6557,    0.7060,    0.4387,
// 0.9058,    0.9706,    0.0357,    0.0318,    0.3816,
...
flmm.setY(y);
flmm.init();
flmm.trainNullModel(500, -5, 5);
double opt_ldelta0 = flmm.get_ldelta0();
double nllmin = flmm.get_nllmin();
EXPECT_NEAR(opt_ldelta0, -5.0, 1e-3);
EXPECT_NEAR(nllmin, 5.26087804791, 1e-3);
}

TEST(LMM, Train_train){
FaSTLMM flmm = FaSTLMM();
// MatrixXd X(10, 5);
// X << 0.8147,    0.1576,    0.6557,    0.7060,    0.4387,
// 0.9058,    0.9706,    0.0357,    0.0318,    0.3816,
...
flmm.init();
flmm.train(500, -5, 5);
MatrixXd SUX = flmm.get_SUX();
MatrixXd SUy = flmm.get_SUy();
MatrixXd SUX0 = flmm.get_SUX0();
MatrixXd SUX_true(2, 2);
MatrixXd SUy_true(2, 1);
MatrixXd SUX0_true(2, 1);
SUX_true << 1.153406, 1.153406,
            1.99329591, 0.99664796;
SUy_true << 3.460218,
            0.99664796;
SUX0_true << 1.153406,
             0.99664796; // should be 0.9966..
TEST_MATRIX_NEAR(SUX, SUX_true, 1e-3);
TEST_MATRIX_NEAR(SUy, SUy_true, 1e-3);
TEST_MATRIX_NEAR(SUX0, SUX0_true, 1e-3);
VectorXd p = flmm.hypothesis_test(SUX, SUy, X, SUX0);
VectorXd p_true;
p_true.resize(2);
p_true << 1, 0.20071408574189295;
