...
long FaSTLMM::getSampleNum() {
    return ns;
}

long FaSTLMM::getFeatureNum() {
    return nf;
}
...
void FaSTLMM::decompose() {
    ...
    // S = SV;
    // cout << SV.rows() << endl;
    // int diff = U.rows() - SV.rows(); // # SNPs < sample size
    // cout << diff << endl;
    // if (diff > 0) {
    //     MatrixXd pad = MatrixXd::Zero(diff, 1);
    //     S << SV, 
    //          pad;
    // }
    // cout << "S" << endl;
...
// void FaSTLMM::calculate_beta(double lambda) {
